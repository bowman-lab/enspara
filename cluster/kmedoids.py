...
def _kmedoids_update(
    ...
    proposed_center_inds, proposed_assignments, proposed_distances = assign_to_nearest_center(traj, proposed_cluster_centers, distance_method)
    
    mean_orig_dist_to_center = distances.dot(distances)/n_frames
    mean_proposed_dist_to_center = proposed_distances.dot(proposed_distances)/n_frames
...
def kmedoids(
    ...
    for i in range(n_iters):
        cluster_center_inds, assignments, distances = _kmedoids_update(traj, distance_method, cluster_center_inds, assignments, distances, verbose=verbose)

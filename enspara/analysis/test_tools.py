...
class Test_Tools(unittest.TestCase):
    ...
    def test_state_relative_entropy(self):
        # reference distributions
        P_test = np.array(
            [
                [0.5, 0.5, 0],
        ...
        assert_array_almost_equal(
            state_relative_entropy(
                P_test, assignments=assignments, prior_counts=0),
            rel_ent_without_prior, 6)
        assert_array_almost_equal(
            state_relative_entropy(P_test, assignments=assignments),
        ...
        assert_array_almost_equal(
            state_relative_entropy(P_test, Q=raw_Q), rel_ent_without_prior, 6)
        assert_array_almost_equal(
            state_relative_entropy(P_test, Q=Q_with_prior),
            rel_ent_with_prior, 6)
        assert_array_almost_equal(
            state_relative_entropy(P_test, Q=Q_transpose_w_prior),
            rel_ent_transpose_with_prior, 6)
    ...
    def test_relative_entropy(self):
        ...
        assert_almost_equal(
            relative_entropy(P_test, assignments=assignments, prior_counts=0),
            rel_ent_without_prior, 7)
        assert_almost_equal(
            relative_entropy(P_test, assignments=assignments),
        ...
        assert_almost_equal(
            relative_entropy(P_test, Q=raw_Q), rel_ent_without_prior, 7)
        assert_almost_equal(
            relative_entropy(P_test, Q=Q_with_prior), rel_ent_with_prior, 7)
        assert_almost_equal(
            relative_entropy(P_test, Q=Q_transpose_w_prior),
            rel_ent_transpose_with_prior, 7)

...
def psi_rotamers(traj, buffer_width=15):
    ...
    rotamers = np.zeros((n_frames,n_angles))
    for i in range(n_angles):
        rotamers[:,i] = _rotamers(shifted_angles[:,i], hard_boundaries, buffer_width)
...
def chi_rotamers(traj, buffer_width=15):
    # could make a dictionary of boundaries for different residue/dihedral types
...
def all_rotamers(traj, buffer_width=15):
    phi_rotameric_states, phi_atom_inds, n_phi_states = phi_rotamers(traj, buffer_width=buffer_width)
    all_rotamers, all_atom_inds, all_n_states = phi_rotameric_states, phi_atom_inds, n_phi_states
    
    psi_rotameric_states, psi_atom_inds, n_psi_states = psi_rotamers(traj, buffer_width=buffer_width)
    ...
    chi_rotameric_states, chi_atom_inds, n_chi_states = chi_rotamers(traj, buffer_width=buffer_width)

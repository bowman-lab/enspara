...
def get_pocket_cells(struct, grid_spacing=0.1, distance_cutoff=0.24, min_rank=3):
    ...
    for i in range(struct.top.n_atoms):
        ...
        min_x_cell_ind, max_x_cell_ind, min_y_cell_ind, max_y_cell_ind, min_z_cell_ind, max_z_cell_ind = _get_cell_inds_within_cutoff(grid, coord, distance_cutoff)
        ...
        grid_inds_within_cutoff = (subgrid_inds_within_cutoff[0]+min_x_cell_ind,
    ...
    # rank each remaining pocket cell based on the number of scans that pass through protein on each side
    ...
    # for each of 4 diagonals diagonals, need scan out from 3 faces of grid
    # along pos x, pos y, pos z
